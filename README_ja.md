...
**shohei** v2.2.0 — **140 MCP ツール × 56 モジュール**搭載 Rust インフラ診断ライブラリ。包括的なセキュリティ・OSINT・脅威インテリジェンス・ガバナンス機能。DNS・TLS・メールセキュリティ・DNS伝播・IPv6 デュアルスタック・セキュリティヘッダを自動検査。**API キー不要 — 完全無料・オープン API のみ使用**。Rust プロジェクトへの組込みも、AI エージェント連携も対応。

- **Claude MCP サーバー** — Claude Desktop から **140 個のツール**を呼び出し可能。「example.com のセキュリティをチェック」と聞けば TLS・メール・DNS・IPv6・RPKI・脅威インテリジェンスを自動診断
...
shohei v2.2.0 は、最も包括的で無料・API キー不要の MCP セキュリティサーバーです：

| 機能 | shohei | honeylabs | kastell | unphurl | cloud-audit | maigret |
|------|:------:|:---------:|:-------:|:-------:|:-----------:|:-------:|
| **MCP ツール数** | **140** | ~25 | ~30 | ~15 | ~20 | ~35 |
| **モジュール数** | **56** | ~8 | ~10 | ~5 | ~7 | ~12 |
| **DNS/DNSSEC** | ✓ | ✓ | ✓ | | | |
| **TLS/証明書** | ✓ | ✓ | ✓ | | | |
| **メールセキュリティ** | ✓ | | | | | |
| **OSINT/偵察** | ✓ | ✓ | | ✓ | | ✓ |
...
**主な利点：**
- **140 MCP ツール** — 最大級の包括的セキュリティツールキット（v2.2.0）
- **API キー不要** — すべてのツールが無料/オープン公開 API を使用
- **56 モジュール** — DNS、TLS、メール、OSINT、脅威インテリ、ガバナンス、仮想通貨、Web セキュリティ、コンプライアンス
...
**MCP（Model Context Protocol）サーバー**（140 ツール）により、Claude Desktop と他の AI エージェントが shohei 診断を直接呼び出し可能です：
...
**Claude が使用可能な 140 個のツール（56 モジュール）：**
